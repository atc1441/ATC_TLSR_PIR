...
RAM uint8_t ble_name[] = {11, 0x09, 'P', 'I', 'R', '_', '0', '0', '0', '0', '0', '0'};
...
RAM uint16_t curr_adv_intervall = 0;
_attribute_ram_code_ void app_switch_to_indirect_adv(uint16_t intervall)
{
	if (curr_adv_intervall != intervall)
	{
		curr_adv_intervall = intervall;
		bls_ll_setAdvParam(intervall, intervall + 10, ADV_TYPE_CONNECTABLE_UNDIRECTED, OWN_ADDRESS_PUBLIC, 0, NULL, BLT_ENABLE_ADV_ALL, ADV_FP_NONE);
...
_attribute_ram_code_ void ble_set_sleep_wakeup(u8 e, u8 *p, int n)
{
	if (gpio_read(BUTTON_IN) == 0)
		cpu_set_gpio_wakeup(BUTTON_IN, Level_High, 1);
	else
		cpu_set_gpio_wakeup(BUTTON_IN, Level_Low, 1);
	bls_pm_setWakeupSource(PM_WAKEUP_PAD);
	if (gpio_read(PIR_IN) == 0)
		cpu_set_gpio_wakeup(PIR_IN, Level_High, 1);
	else
		cpu_set_gpio_wakeup(PIR_IN, Level_Low, 1);
}

extern uint32_t last_GPIO_second;
extern uint8_t should_update_adv;
RAM long last_gpio_wakeup = 0;
_attribute_ram_code_ void ble_gpio_wakeup(u8 e, u8 *p, int n)
{
	set_led_color(1);
	printf("GPIO Wakeup\r\n");
	printf("Read GPIO: %02X %02X %d\r\n", gpio_read(BUTTON_IN) ? 1 : 0, gpio_read(PIR_IN) ? 1 : 0, get_time());
	should_update_adv = 1;
	last_GPIO_second = get_time();
	// if (ble_connected == 0)
	//	app_switch_to_indirect_adv(600);
	set_led_color(0);
}
...
void init_ble(void)
{
	...
	bls_app_registerEventCallback(BLT_EV_FLAG_SUSPEND_ENTER, &ble_set_sleep_wakeup);
	bls_app_registerEventCallback(BLT_EV_FLAG_GPIO_EARLY_WAKEUP, &ble_gpio_wakeup);
...
_attribute_ram_code_ void set_adv_data(int16_t temp, uint8_t pir, uint8_t battery_level, uint16_t battery_mv)
{
	// advertising_data[10] = temp >> 8;
	// advertising_data[11] = temp & 0xff;

	if (pir != advertising_data[12])
	{
		uint16_t temper = (advertising_data[10] << 8) | advertising_data[11];
		temper++;
		advertising_data[10] = temper >> 8;
		advertising_data[11] = temper & 0xff;
	}

	advertising_data[12] = pir;
